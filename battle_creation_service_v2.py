...
def sync_to_client(force=False):
    ...
    """
    print("� [Sync] Synchronisation COMPLÈTEMENT désactivée")
    return
...
class BattleCreationServiceV2:
    ...
    def __init__(self):
        ...
        # Nous voulons aller jusqu'à .../server/data
        self.data_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'data')
        self.savegame_path = os.path.join(self.data_dir, 'savegame.json')
        self.battlefields_v2_path = os.path.join(self.data_dir, 'battlefields_v2.json')
        ...
        # Gestionnaire de groupes de murs
        self.wall_group_manager = get_wall_group_manager(self.data_dir)
    ...
    def _load_savegame(self) -> Dict[str, Any]:
        """Charge le fichier savegame.json"""
        try:
            with open(self.savegame_path, 'r', encoding='utf-8') as f:
                return json.load(f)
        except Exception as e:
            print(f"❌ [V2] Erreur lors du chargement du savegame: {e}")
            return {}
    
    def _save_savegame(self, data: Dict[str, Any]):
        """Sauvegarde le fichier savegame.json"""
        try:
    ...
    def _load_battlefields_v2(self) -> Dict[str, Any]:
        ...
        try:
            ...
            wall_manager = WallGroupManager(self.data_dir)
            ...
            print(f"❌ [V2] Erreur lors du chargement des battlefields V2: {e}")
            return {}
    ...
    def _save_battlefields_v2(self, data: Dict[str, Any]):
        ...
        try:
            ...
            print("✅ [V2] Battlefields V2 sauvegardés avec positions compactées")
            ...
            print(f"❌ [V2] Erreur lors de la sauvegarde des battlefields V2: {e}")
    ...
    def _sync_unit_counts_for_battles(self, data: Dict[str, Any]):
        ...
        try:
            ...
            battles_v2_path = os.path.join(self.data_dir, 'battlesv2.json')
            ...
            for battle_id in data.keys():
                if battle_id in battles_data:
                    ...
                    if new_unit_counts:
                        ...
                        updated = True
                        print(f"✅ [SYNC] Unit_counts synchronisés pour {battle_id} (round {current_round}, joueur {current_player} préservés)")
            ...
            if updated:
                if save_json_data(BATTLES_V2_FILE, battles_data, compact=True):
                    print("✅ [SYNC] Battles V2 mis à jour avec nouveaux unit_counts (format compact)")
                else:
                    print("❌ [SYNC] Erreur lors de la sauvegarde")
            
        except Exception as e:
            print(f"❌ [SYNC] Erreur synchronisation unit_counts: {e}")
    ...
    def _get_wall_level_for_city(self, city_id: str, savegame_data: Dict[str, Any]) -> int:
        ...
        try:
            ...
            print(f"❌ Erreur récupération niveau muraille pour {city_id}: {e}")
            return 0
    ...
    def _get_city_owner(self, city_id: str, savegame_data: Dict[str, Any]) -> str:
        ...
        if city_id.startswith('barbarian_village_'):
            return 'barbarian_village'  # Identifiant spécial pour les villages barbares
    ...
    def _deduct_troops_from_city(self, city_id: str, units_to_deduct: Dict[str, int], savegame_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            print(f"❌ [V2] Erreur lors de la déduction des troupes: {e}")
            return False
    ...
    def _get_and_deduct_city_heroes(self, city_id: str, savegame_data: Dict[str, Any], player_id: str) -> list:
        ...
        try:
            for city in savegame_data.get('cities', []):
                if city['id'] == city_id:
                    ...
                    if isinstance(heroes_section, list):
                        print(f"👑 [V2] Pas de héros dans {city_id} (liste vide)")
                        return []
                    ...
                    if isinstance(heroes_section, dict):
                        for hero_id, hero_data in heroes_section.items():
                            if (hero_data.get('owner') == player_id and 
                                ...
                                print(f"👑 [V2] Héros {hero_id} de {city_id} marqué en bataille")
            ...
            print(f"❌ [V2] Erreur lors de la gestion des héros: {e}")
            return []
    ...
    def _deduct_attacker_heroes(self, city_id: str, heroes: list, savegame_data: Dict[str, Any], player_id: str) -> bool:
        ...
        try:
                ...
                if city['id'] == city_id:
                    ...
                    for hero_id in heroes:
                        if (hero_id not in heroes_section or 
                            ...
                            print(f"❌ [V2] Héros {hero_id} non disponible pour {player_id}")
                            return False
                    
                    # Marquer les héros comme en bataille
                    for hero_id in heroes:
                        heroes_section[hero_id]['status'] = 'en_bataille'
                        print(f"👑 [V2] Héros attaquant {hero_id} marqué en bataille")
            ...
            print(f"❌ [V2] Erreur lors de la déduction des héros attaquants: {e}")
            return False
    ...
    def _load_players_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"❌ [V2] Erreur lors du chargement des joueurs: {e}")
            return {}
    ...
    def _save_players_data(self, data: Dict[str, Any]):
        ...
        try:
            ...
            with open(players_path, 'w', encoding='utf-8') as f:
                json.dump(data, f, indent=2, ensure_ascii=False)
            print("✅ [V2] Données des joueurs sauvegardées")
        except Exception as e:
            print(f"❌ [V2] Erreur lors de la sauvegarde des joueurs: {e}")
    ...
    def _check_and_deduct_ships(self, player_id: str, ships_needed: int) -> bool:
        ...
        try:
            ...
            if not player:
                print(f"❌ [V2] Joueur {player_id} non trouvé")
                return False
            
            # Vérifier les bateaux disponibles
            ships_total = player.get('transport_ships_total', 0)
            if ships_total < ships_needed:
                print(f"❌ [V2] Pas assez de bateaux pour {player_id}: {ships_total} disponibles, {ships_needed} requis")
                return False
            
            # Déduire les bateaux
            player['transport_ships_total'] = ships_total - ships_needed
            print(f"🚢 [V2] {ships_needed} bateaux déduits de {player_id}: {ships_total} → {player['transport_ships_total']}")
            ...
            print(f"❌ [V2] Erreur lors de la déduction des bateaux: {e}")
            return False
        ...
        try:
            ...
            for contribution in contributions:
                ...
                for hero_id in heroes:
                    ...
                    print(f"   🦸 Héros {hero_id}: +{hero_bonus} moral")
            
            return total_hero_bonus
            
        except Exception as e:
            print(f"⚠️ [CREATION] Erreur calcul bonus héros pour {player_id}: {e}")
            return 0
    ...
    def _get_hero_moral_bonus_by_id(self, hero_id: str, player_id: str) -> int:
        ...
        """
        try:
            player_heroes_path = os.path.join(self.data_dir, 'player_heroes.json')
            ...
            print(f"⚠️ [CREATION] Erreur lecture bonus moral héros {hero_id}: {e}")
            return 0
    ...
    def _transition_to_battle_ready(self, battle_id: str):
        ...
        print(f"⚠️ [V2] _transition_to_battle_ready appelée inutilement pour {battle_id} - battlefield déjà prêt")
    ...
    def create_battle(self, attacker_city_id: str, target_city_id: str, units: Dict[str, int], 
        ...
        try:
            ...
            if existing_battle_id:
                print(f"� [V2] Bataille existante trouvée: {existing_battle_id} - Ajout de renforts")
                ...
                print(f"� [V2] Aucune bataille existante - Création nouvelle bataille")
                ...
                print(f"🗺️ [V2] Template battlefield déterminé: {correct_template}")
            ...
            print(f"❌ [V2] Erreur lors de la création/ajout de renforts: {e}")
            return {
                "success": False,
                "error": str(e)
            }
    ...
    def get_battlefield(self, battle_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Nouvelle structure: accès direct par battle_id
            return battlefields_data.get(battle_id)
            
        except Exception as e:
            print(f"❌ [V2] Erreur lors de la récupération du battlefield {battle_id}: {e}")
            return None
    ...
    def get_all_battlefields(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"❌ [V2] Erreur lors de la récupération de tous les battlefields: {e}")
            return {}
    ...
    def get_battlefields_by_status(self, status: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"❌ [V2] Erreur lors de la récupération des battlefields {status}: {e}")
            return {}
    ...
    def get_player_active_battles(self, player_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"🔍 [V2] Batailles trouvées pour {player_id}: {len(player_battles['transport'])} transport, {len(player_battles['battle_ready'])} battle_ready, {len(player_battles['completed'])} completed")
            return player_battles
            
        except Exception as e:
            print(f"❌ [V2] Erreur lors de la récupération des batailles du joueur {player_id}: {e}")
    ...
    def cleanup_old_completed_battles(self, max_age_hours: int = 24):
        ...
        try:
            ...
            for battle_id in battles_to_remove:
                ...
                print(f"🗑️ [V2] Bataille ancienne supprimée: {battle_id}")
            
            if battles_to_remove:
                self._save_battlefields_v2(battlefields_data)
                print(f"✅ [V2] {len(battles_to_remove)} anciennes batailles nettoyées")
            
        except Exception as e:
            print(f"❌ [V2] Erreur lors du nettoyage des anciennes batailles: {e}")
    ...
    def get_battlefield_status(self, battle_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"❌ [V2] Erreur lors de la récupération du statut {battle_id}: {e}")
            return {
                "success": False,
                "error": str(e)
            }
        ...
        for battle_id, battlefield in battlefields_data.items():
            if battlefield.get('location') == target_city_id:
                ...
                if status in ['transport', 'battle_ready']:
                    print(f"🔍 [V2] Bataille existante trouvée: {battle_id} (status: {status})")
                    return battle_id
        return None
        ...
        try:
            ...
            if attacker_player_id in battlefield['forces']['attackers']:
                ...
                if transport_id:
                    for existing_contrib in existing_contributions:
                        if existing_contrib.get('id') == transport_id:
                            ...
                            print(f"⚠️ [V2] Contribution {transport_id} déjà existante - ignorée")
                            break
                ...
                for unit_type, count in units.items():
                    if unit_type in battlefield['forces']['attackers'][attacker_player_id]['units_lost']:
                        # Garder les pertes existantes, ne pas les réinitialiser
                        pass
                    else:
                        battlefield['forces']['attackers'][attacker_player_id]['units_lost'][unit_type] = 0
                        
                print(f"✅ [V2] Contribution ajoutée au joueur existant {attacker_player_id}")
            else:
                ...
                print(f"✅ [V2] Nouveau joueur {attacker_player_id} ajouté aux attaquants")
            ...
            print(f"✅ [V2] Renforts ajoutés à {existing_battle_id} pour le joueur {attacker_player_id}")
            ...
            print(f"❌ [V2] Erreur lors de l'ajout de renforts: {e}")
    ...
    def _create_new_battle(self, attacker_city_id: str, target_city_id: str, units: dict,
        ...
        try:
            ...
            print(f"👥 [V2] Attaquant: {attacker_player_id}, Défenseur: {defender_player_id}")
            ...
            if target_city_id.startswith('barbarian_village_'):
                print(f"🏺 [V2] Création bataille contre village barbare: {target_city_id}")
                
                # Récupérer l'île et le niveau du village barbare
                island_id = target_city_id.replace('barbarian_village_', '')
                
                # Récupérer le niveau du village barbare pour l'attaquant
                barbarian_level = 1  # Niveau par défaut
                for city in savegame_data.get('cities', []):
                    if city['id'] == attacker_city_id:
                        barbarian_level = city.get('barbarian_village_level', 1)
                        break
                
                # Charger les unités barbares depuis la configuration
                try:
                    import json
                    barbarian_config_path = os.path.join(self.data_dir, 'barbarian_villages_config.json')
                    ...
                    if level_key in barbarian_config:
                        ...
                        print(f"🏺 [V2] Unités barbares niveau {barbarian_level}: {barbarian_units}")
                        
                        # Créer la contribution défensive barbare
                        barbarian_contribution = {
                            "player_id": "barbarian_village",
                        ...
                        defenders_data['barbarian_village'] = {
                        ...
                        defender_participants = ['barbarian_village']
                    else:
                        print(f"❌ [V2] Niveau barbare {barbarian_level} non trouvé dans la configuration")
                        defender_participants = ['barbarian_village']
                        defenders_data['barbarian_village'] = {
                            "units_lost": {},
                            "units_killed": {},
                    ...
                    print(f"❌ [V2] Erreur chargement unités barbares: {e}")
                    defender_participants = ['barbarian_village']
                    defenders_data['barbarian_village'] = {
                        "units_lost": {},
                        "units_killed": {},
                ...
                for city in savegame_data.get('cities', []):
                    if city['id'] == target_city_id:
                        garrison = city.get('military', {}).get('garrison', {})
                        print(f"🏰 [V2] Garrison complète de {target_city_id}: {garrison}")
                        ...
                        for player_id, player_units in garrison.items():
                            ...
                            if units_for_battle:
                                ...
                                print(f"✅ [V2] Défenseur {player_id} DÉDUIT: {units_for_battle}")
                        ...
                        print(f"🏰 [V2] Garrison après déduction: {garrison}")
                        break
            
            # Si aucune garrison, défenseur par défaut
            if not defender_participants:
                defender_participants = [defender_player_id]
                print(f"⚠️ [V2] Aucune garrison, défenseur par défaut: {defender_player_id}")
            ...
            if target_city_id.startswith('barbarian_village_') and 'barbarian_level' in locals():
            ...
            if wall_level > 0:
                try:
                    ...
                    if wall_data:
                        ...
                        print(f"🧱 [V2] Murs initialisés pour bataille {battle_id} (niveau {wall_level})")
                except Exception as e:
            ...
            print(f"✅ [V2] Bataille {battle_id} créée directement en statut battle_ready")
            print(f"👥 [V2] Défenseurs: {list(defenders_data.keys())}")
            ...
            print(f"❌ [V2] Erreur lors de la création de bataille: {e}")
            return {"success": False, "error": str(e)}
    ...
    def _generate_unit_counts_from_battlefield(self, battle_id: str) -> dict:
        ...
        """
        try:
            battlefields_file = os.path.join(self.data_dir, 'battlefields_v2.json')
            ...
            print(f"✅ [V2] Unit_counts générés pour {battle_id}: {unit_counts}")
            return unit_counts
    ...
    def _create_base_battle_entry(self, battle_id: str, timestamp: int, location: str):
        ...
        """
        try:
            battles_file = os.path.join(self.data_dir, 'battlesv2.json')
            ...
            if battle_id not in battlefields_data:
                print(f"❌ [V2] Battlefield {battle_id} non trouvé pour créer l'entrée de base")
                return
            ...
            try:
                ...
                print(f"🔧 [V2] Utilisation format compact pour {battle_id}")
                
                with open(battles_file, 'w', encoding='utf-8') as f:
                    json_str = save_battles_ultra_compact(battles_data)
                    f.write(json_str)
            except ImportError:
                print(f"⚠️ [V2] Fallback format étendu pour {battle_id} - import échoué")
                with open(battles_file, 'w', encoding='utf-8') as f:
                    json.dump(battles_data, f, ensure_ascii=False, indent=2)
            
            print(f"✅ [V2] Entrée de base créée dans battlesv2.json pour {battle_id}")
            
            # Synchronisation désactivée (sera forcée seulement pour les renforts)
            sync_to_client(force=False)
            
        except Exception as e:
            print(f"⚠️ [V2] Erreur création entrée de base: {e}")
