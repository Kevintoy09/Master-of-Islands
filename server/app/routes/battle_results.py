...
# Cache pour réduire la charge des lectures de fichiers
@lru_cache(maxsize=1)
def load_battlefields_cached():
    """Charger les données des battlefields avec cache (invalidé toutes les 30 secondes)"""
    return load_battlefields()
        ...
        for battlefield_id, battlefield in battlefields.items():
            ...
            if is_participant:
                ...
                if destination.startswith('wild_camp_'):
                    destination = f"Camp des Sauvages {destination.split('_')[-1]}"
...
@battles_bp.route('/api/v2/battles/check-player/<player_id>', methods=['GET'])
def check_player_has_battles(player_id):
    """Vérifier si un joueur a des batailles actives"""
    try:
        # Charger directement sans cache pour avoir les données à jour
        battlefields = load_battlefields()
        
        # Vérifier s'il y a au moins une bataille pour ce joueur
        for battlefield_id, battlefield in battlefields.items():
            participants = battlefield.get('participants', {})
            ...
            if player_id in attackers or player_id in defenders:
                return jsonify({
                    'has_active_battles': True
                })
        
        return jsonify({
            'has_active_battles': False
        })
        
    except Exception as e:
        print(f"Erreur lors de la vérification des batailles pour {player_id}: {str(e)}")
        return jsonify({
            'has_active_battles': False,
            'error': str(e)
        }), 500
